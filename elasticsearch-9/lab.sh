...
Lifecycle:
  doctor|check       Check host prerequisites and resolved topology
...
Notes:
  - Security is OFF by default (unauthenticated, HTTP). Setting
    XPACK_SECURITY_ENABLED=true additionally requires transport TLS (see README)
    and is not configured by this lab.
...
Quick start:
  cp .env.example .env        # then edit ports/provider if needed
  ./lab.sh up
  ./lab.sh doctor
  ./lab.sh snapshot
