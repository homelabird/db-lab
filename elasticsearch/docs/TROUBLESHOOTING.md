...
브라우저를 다른 PC에서 열었다면 그 PC의 localhost와 서버의 localhost는 다릅니다. 기본
바인딩은 `127.0.0.1`이므로 서버에서만 접속할 수 있습니다. 원격 접속은 [QUICKSTART.md](QUICKSTART.md)의
SSH 터널을 권장합니다.

격리된 네트워크에서 직접 공개하기로 했다면 `.env`에 `ES_BIND_IP=0.0.0.0`과
`ES_ALLOW_PUBLIC_BIND=yes`를 설정합니다. `.env` 설정 후 `./lab.sh up`으로 적용하세요.
호스트 방화벽/클라우드 보안그룹은 신뢰하는 클라이언트 IP만 허용해야 합니다. 브라우저
주소에 `0.0.0.0` 또는 `0.0.0.0/0`을 입력하지 마세요.
