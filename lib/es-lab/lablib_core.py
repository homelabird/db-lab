...
class ESClient:
    ...
    def ensure_snapshot_repo(self, name, location, compress=True):
        ...
        if not nodes:
            raise RuntimeError(f'Snapshot repository {name!r} verified on no nodes: {json.dumps(verified)[:2000]}')
